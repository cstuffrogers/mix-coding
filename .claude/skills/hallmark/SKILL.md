...
The per-theme axis values live in the **Axes (diversification)** section of each theme's file under [`references/themes/`](references/themes/). When in doubt, name your candidate theme out loud and identify its three axis values; if two of three match the previous output, redirect.
...
- Worked briefs and DNA-extraction examples for human readers live alongside the verb references under [`references/verbs/`](references/verbs/) and [`study.md`](references/study.md); load them only when hand-writing a brief or extraction.
