...
| `/audit` | `> /audit` | 22步 | 一键全量项目审计：安全扫描 + 代码质量 + 依赖审计 + 性能基线 + 覆盖率 + 质量门禁汇总 |
...
| `/hunt` | `> /hunt` | 14步 | 代码安全漏洞扫描与修复（集成CE知识沉淀） |
...
| `/release` | `> /release` | 19步 | 端到端发布部署流程：质量门禁 → 版本号 → 构建 → 验证 → Tag → 发布 |
| `/review` | `> /review` | 12步 | 全面代码质量审查 + 自动化安全漏洞扫描（集成CE多Agent深度审查 + sec-bug-hunt） |
